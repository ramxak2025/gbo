...
export function DataProvider({ children }) {
  ...
  const update = useCallback(async (updater) => {
    ...
    if (next.tournamentRegistrations !== data.tournamentRegistrations) {
      ...
      for (const or2 of oldRegs) {
        if (!newRegs.find(r => r.tournamentId === or2.tournamentId && r.studentId === or2.studentId)) {
          await api.unregisterTournament(or2.tournamentId, or2.studentId);
        }
      }
    }
