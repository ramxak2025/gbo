    ...
    <Layout>
      ...
      <div className="px-4 space-y-4 slide-in">
        {tournament.coverImage ? (
          ...
          <div className={`w-full h-44 rounded-[24px] flex items-center justify-center ${dark ? 'bg-white/5' : 'bg-black/[0.03]'}`}>
      ...
      <Modal open={editing} onClose={() => setEditing(false)} title="Редактировать турнир">
        {form && (
          <form onSubmit={saveEdit} className="space-y-3">
            <label className={`block w-full h-32 rounded-[16px] cursor-pointer press-scale flex items-center justify-center overflow-hidden ${dark ? 'bg-white/5 border border-dashed border-white/20' : 'bg-black/[0.03] border border-dashed border-black/[0.15]'}`}>
              {form.coverImage
            ...
            <input type="date" value={form.date} onChange={e => setForm(f => ({ ...f, date: e.target.value }))} className={`w-full px-2 py-1.5 rounded-lg text-xs outline-none ${dark ? 'bg-white/5 border border-white/10 text-white focus:border-accent' : 'bg-black/[0.03] border border-black/[0.08] text-gray-900 focus:border-accent'}`} />
